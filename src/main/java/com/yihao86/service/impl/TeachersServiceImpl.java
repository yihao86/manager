...
public class TeachersServiceImpl implements TeachersService {
	...
	public static MimeMessage createEmail(Session session, Teachers teacher) throws Exception {
		...
		InternetAddress fromAddress = new InternetAddress(ACCOUT, "注册信息", "utf-8");
		// 设置发送邮件方
		msg.setFrom(fromAddress);
		InternetAddress receiveAddress = new InternetAddress("503453843@qq.com", "test", "utf-8");
		// 设置邮件接收方
		 ...
		 * +
		 * "rel=\"noopener\" target=\"_blank\">确认注册信息</a></td></tr></tbody></table></td>"
		...
		msg.setContent("<div style=\"text-align:center;\" ><tbody ><tr style=\"width:685px;\" class=\"devWidth\">\n"
				...
				+ teacher.getT_email() + "<p>" + "		   <p>简介信息 ：" + teacher.getT_abstract() + "<p>"
				+ "						</td></tr>\n" + "					\n"
	...
	public Map<String, Object> selectVideosByVid(int vid) {
		...
		Videos v = findVideoByName(vid);
		String v_vpath = v.getV_vpath();

		String path = v_vpath.substring(0, v_vpath.lastIndexOf("/"));

		String v_path = path.substring(path.lastIndexOf("/") + 1);

		map = dao.findVideoByVid(vid);
		map.put("zname", v_path);
	 ...
	 */
	@Override
	public int updateByVid(Videos videos, MultipartFile files) throws Exception {
		Videos v = findVideoByName(videos.getVid());
		String fileName = v.getV_imgs();		
		String file = files.getOriginalFilename();
		if (files.isEmpty()) {
			file = fileName;
			videos.setV_imgs(file);
		} else {	
			System.out.println(file);
			String newfile = fileName.substring(0, fileName.lastIndexOf("/") + 1); // 获取原路径
			newfile = URLDecoder.decode(newfile, "utf-8");
			 String uuid = UUID.randomUUID().toString();	
			// 获取uuid最后一截值当作新的图片名称	
  			 String uu = uuid.substring(uuid.lastIndexOf("-")+1);
  			//将截取后的uuid替换成图片名
  			String nfile=file.replaceFirst(file.substring(0,file.indexOf(".")), uu);
			// 把图片按照路径写入服务器
			String img_path = fileDir + newfile; // 得到服务器地址
			try {
				File df = new File(img_path, nfile); //根据路径写入文件
				files.transferTo(df);		//写入文件	
				videos.setV_imgs((newfile + nfile));
				
				System.out.println(videos.getV_imgs());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		return dao.updateByIdVideo(videos);
